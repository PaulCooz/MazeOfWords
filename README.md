...
- **Platform (Yandex Games)** — `index.ejs` loads the SDK and a `PlatformReady` promise; `YandexGames.init` awaits it (editor uses `Mock`), reports game ready, and tracks gameplay pause/resume. `Config` holds Remote Config–overridable constants. `Adv` shows fullscreen interstitials with `Config.interCooldown`. The footer button shows a rewarded video and grants `Config.rewardedCoins`. Authorized players submit `leaderboards.setScore("level", displayedLevel)`
